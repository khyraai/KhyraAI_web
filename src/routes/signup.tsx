      ...
      {steps.map((s, i) => (
        <div key={s.n} className="flex items-center">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-medium ${step === s.n ? "text-primary" : step > s.n ? "text-foreground" : "text-muted-foreground"}`}>{s.label}</span>
          </div>
          {i < steps.length - 1 && <div className={`mx-4 h-px w-14 transition-colors ${step > s.n ? "bg-primary" : "bg-border"}`} />}
        </div>
      ))}
    </div>
  );
...
function LeftPanel() {
  ...
  return (
    <div className="relative hidden w-[380px] flex-shrink-0 flex-col overflow-hidden bg-primary px-10 py-10 text-primary-foreground lg:flex">
          ...
          {bullets.map((b) => (
            <div key={b.title} className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs leading-relaxed text-primary-foreground/60">{b.body}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-1 items-start justify-center overflow-y-auto px-8 pb-10 pt-2">
          <div className="w-full max-w-[580px]">
            ...
            {step === 1 && (
              <>
                <div className="mb-6">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">Enter your basic information to get started</p>
                </div>
                ...
                <form onSubmit={handleStep1} className="space-y-4">
                  ...
                  <div className="flex items-center gap-4 pt-1">
                    <div className="my-1 flex items-center gap-3">
                      ...
                      <span className="text-xs uppercase tracking-wider text-muted-foreground">or sign up with</span>
                      <div className="h-px w-20 bg-border" />
                    </div>
                  </div>

                  <button type="button" onClick={handleGoogle} className="flex w-full items-center justify-center gap-3 rounded-xl border border-border bg-background py-3 text-sm font-medium text-foreground transition hover:bg-secondary active:scale-[0.98]">
                    {GOOGLE_ICON}<span>Sign up with Google</span>
                  </button>

                  <div className="flex justify-end pt-2">
                    <button type="submit" className="inline-flex items-center gap-2 rounded-xl bg-primary px-8 py-3 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90 active:scale-[0.98]">
            ...
            {step === 2 && (
              <>
                <div className="mb-6">
                  <h1 className="font-display text-2xl text-ink">Additional Information</h1>
                  <p className="mt-1 text-sm text-muted-foreground">Tell us more about yourself. Fields marked <span className="text-red-500">*</span> are required.</p>
                </div>
                {authError && <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">{authError}</div>}
                <form onSubmit={handleStep2} className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <F label="Phone number" req error={s2.formState.errors.phone?.message}>
                      <div className="flex gap-2">
                        <div className="flex items-center rounded-xl border border-border bg-background px-3 text-sm text-muted-foreground">+91 IN</div>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <button type="button" onClick={() => setStep(1)} className="inline-flex items-center gap-1.5 rounded-xl border border-border px-5 py-3 text-sm font-medium text-foreground transition hover:bg-secondary">
            ...
            {step === 3 && (
              <div className="py-4 text-center">
                ...
                <p className="mt-0.5 text-sm font-semibold text-primary">{userEmail}</p>

                <div className="mx-auto mt-6 max-w-sm rounded-xl border border-border bg-secondary/40 px-6 py-5 text-left">
                  <p className="text-sm font-medium text-foreground">How to verify:</p>
                ...
                {resendSent && (
                  <p className="mt-3 text-xs text-primary">Verification email resent!</p>
                )}

                <div className="mt-6 flex items-center justify-center gap-3">
                  <button type="button" onClick={() => setStep(2)} className="inline-flex items-center gap-1.5 rounded-xl border border-border px-5 py-3 text-sm font-medium text-foreground transition hover:bg-secondary">
                    <ChevronLeft className="h-4 w-4" /> Back
                  </button>
                  <button type="button" onClick={handleVerify} disabled={verifying} className="inline-flex items-center gap-2 rounded-xl bg-primary px-8 py-3 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90 disabled:opacity-60">
                    {verifying ? "Checking…" : "I've Verified"} {!verifying && <Check className="h-4 w-4" />}
                  </button>
                </div>

                <button type="button" onClick={handleResend} disabled={resendLoading} className="mt-4 text-sm text-muted-foreground underline-offset-4 hover:text-primary hover:underline disabled:opacity-50">
